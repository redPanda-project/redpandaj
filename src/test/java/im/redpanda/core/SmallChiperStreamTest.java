...
public class SmallChiperStreamTest {
  ...
  public void runExperimentsTwo()
    ...
    try (CipherOutputStreamByteBuffer output =
            new CipherOutputStreamByteBuffer(byteArrayOutputStream, c);
        PrintWriter pw = new PrintWriter(output)) {
  ...
  public static class MyInputStream extends InputStream {
    ...
    public int read(@NotNull byte[] b, int off, int len) throws IOException {
      throw new RuntimeException(
          "not implemented by now, maybe this is not needed.... code: 4hdfg5tj5n");
      // if (buffers.isEmpty()) {
      // return -1;
      // }
      //
