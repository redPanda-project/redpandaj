...
class InboundCommandProcessorEvenMoreCoverageTest {
  ...
  void sendPeerList_parsesEntries_addsPeers_skipsInvalidIpAndSelf() {
        ...
        PeerInfoProto.newBuilder()
            ...
            .setNodeId(
                NodeIdProto.newBuilder()
                    .setPublicKeyBytes(copyFrom(otherNode.exportPublic()))
                    .build())
            .build();

    // Entry 2: no nodeId + valid ip
    PeerInfoProto p2 = PeerInfoProto.newBuilder().setIp("10.10.0.2").setPort(2222).build();

    SendPeerList spl = SendPeerList.newBuilder().addPeers(p1).addPeers(p2).build();
    ...
    assertNotNull(ctx.getPeerList().getByAddress("10.10.0.2", 2222));
  }
  ...
  void kademliaGet_miss_startsSearch_returnsConsumed() {
    ...
    KademliaGet getMsg =
        KademliaGet.newBuilder()
            .setJobId(123456)
            .setSearchedId(KademliaIdProto.newBuilder().setKeyBytes(copyFrom(randomId)).build())
            .build();
