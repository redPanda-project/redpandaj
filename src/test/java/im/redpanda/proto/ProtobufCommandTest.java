...
public class ProtobufCommandTest {
  ...
  public void testKademliaIdSerialization() {
    ...
    KademliaIdProto proto =
        KademliaIdProto.newBuilder().setKeyBytes(ByteString.copyFrom(originalBytes)).build();
  ...
  public void testSendPeerListSerialization() {
    ...
    SendPeerList sendPeerList = SendPeerList.newBuilder().addPeers(peer).build();
    ...
    try {
      ...
      assertArrayEquals(
          nodeId.exportPublic(), parsedPeer.getNodeId().getPublicKeyBytes().toByteArray());
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
  }
