...
public class Sha256Hash implements Serializable {
  ...
  /**
   * Returns a hash of the given files contents. Reads the file fully into memory before hashing so
  ...
  /**
   * Hash code of the byte array as calculated by {@link Arrays#hashCode()}. Note the difference
   * between a SHA256 secure bytes and the type of quick/dirty bytes used by the Java hashCode
   * method which is designed for use in bytes tables.
   */
  @Override
  public int hashCode() {
    // Use the last 4 bytes, not the first 4 which are often zeros in Bitcoin.
    return (bytes[31] & 0xFF)
        | ((bytes[30] & 0xFF) << 8)
        | ((bytes[29] & 0xFF) << 16)
        | ((bytes[28] & 0xFF) << 24);
  }
