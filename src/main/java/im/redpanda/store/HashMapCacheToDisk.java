...
public class HashMapCacheToDisk<K, V> extends HashMap<K, V> {
  ...
  public HashMapCacheToDisk() {
    ...
    @SuppressWarnings("unchecked")
    HTreeMap<K, V> inMemoryTemp =
        (HTreeMap<K, V>)
            DBMaker.memoryShardedHashMap(16)
                .expireExecutor(Executors.newScheduledThreadPool(3))
                .expireAfterCreate(
                    TTL_IN_MEMORY_OFF_HEAP) // keep TTL_IN_MEMORY_OFF_HEAP mins in off-heap
                // serialized
    ...
    public void run() {
      ...
      while (inMemory != null && onDisk != null && !inMemory.isClosed() && !onDisk.isClosed()) {
        ...
        if (cnt != 0) {
          System.out.println(
              "sleeping "
                  + NUMBER_OF_EVICTION_THREADS * (long) Math.ceil(MEAN_STORAGE_TIME / cnt)
                  + " ms, ojects stored in on-heap: "
                  + cnt
                  + " off-heap: "
                  + inMemory.size()
                  + " on disk: "
